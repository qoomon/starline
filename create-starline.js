...
console.log('Create starline for ' + input.repository)
...
console.log('Write starline image to ' + svgFileNameLocal)
writeFileSyncRecursive(svgFileNameLocal, svg)

// github release store
console.log(`Uploaded starline image to GitHub release ${starlineConfig.cache.releaseTag}`)
...
async function getStargazerDates(repository) {
    if (repository.startsWith('users/')) {
        return await getOwnerStargazerDates(parseRepository(repository).repo)
    }

    console.log('Load stargazer dates cache...')
    const stargazerCache = await loadStargazerDates(repository)
    if (stargazerCache.dates.length > 0) {
        console.log(`  ${stargazerCache.dates.length} stargazers until ${stargazerCache.dates[0].toISOString().split('T')[0]}`)
    } else {
        console.log('  0 stargazers')
    }

    if (stargazerCache.age <= starlineConfig.cache.maxAge) {
        console.log(`  expires in ${humanizeDuration(starlineConfig.cache.maxAge - stargazerCache.age)} (${(starlineConfig.cache.maxAge - stargazerCache.age)})`)
        return {
            cached: stargazerCache.dates.length,
            fetched: 0,
            dates: stargazerCache.dates,
        }
    }

    console.log('Fetch stargazer dates...')
    ...
    if (fetchedStargazerDates.length > 0) {
        console.log(`Cache ${stargazerDates.length} stargazers until ${stargazerDates[0].toISOString().split('T')[0]}`)
...
async function getOwnerStargazerDates(owner) {
    let cached = 0
    let fetched = 0
    let dates = []
    ...
    for (const gist of gists) {
        console.log(gist + '...')
        const stargazerDates = await getStargazerDates(gist)
        cached += stargazerDates.cached
        fetched += stargazerDates.fetched
        dates = dates.concat(stargazerDates.dates)
    }
